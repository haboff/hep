...
class CodeView(QListWidget):
    ...
    def __init__(self, parent=None, notebook=None):
        ...
        codeCheckAction = QAction(
            'Проверка стиля кода (используя pyspellcheck)', self)
        codeCheckAction.triggered.connect(self.onCodeCheck)
        compileAction = QAction('Компилирование файла', self)
    ...
    def onCode(self):
        ...
        if filename:
            try:
                findDeadCode = FindDeadCodeDialog(
                    self.mainWindow, textPad, self)
            ...
            self.mainWindow.statusBar.showMessage(
                "Не удалось найти мертвый код !", 3000)
    ...
    def onCodeCheck(self):
        ...
        if filename:
            ...
            self.mainWindow.statusBar.showMessage(
                "Не удалось проверить код без имени файла !", 3000)
    ...
    def onCompile(self):
        ...
        if filename:
            ...
            self.mainWindow.statusBar.showMessage(
                "Не удалось скомпилировать код без имени файла !", 3000)
