...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setWindowTitle('Human Editor Pro')
        ...
        self.fileBrowser = FileBrowser(
            self, self.textPad, self.notebook, self.codeView)
        ...
        # actions
        newAction = QAction(
            QIcon(self.HOME + 'images/new.png'), 'Новый файл', self)

        newAction.setShortcut('Ctrl+N')
        newAction.triggered.connect(self.new)

        openAction = QAction(
            QIcon(self.HOME + 'images/open.png'), 'Открыть', self)
        openAction.setShortcut('Ctrl+O')
        openAction.triggered.connect(self.open)

        saveAction = QAction(
            QIcon(self.HOME + 'images/save.png'), 'Сохранить', self)
        saveAction.setShortcut('Ctrl+S')
        saveAction.triggered.connect(self.save)

        saveAsAction = QAction(
            QIcon(self.HOME + 'images/saveAs.png'), 'Сохранить как', self)
        saveAsAction.setShortcut('Ctrl+Shift+S')
        saveAsAction.triggered.connect(self.saveAs)

        printAction = QAction(
            QIcon(self.HOME + 'images/print.png'), 'Печать', self)
        printAction.setShortcut('Ctrl+P')
        printAction.triggered.connect(self.onPrint)

        undoAction = QAction(
            QIcon(self.HOME + 'images/undo.png'), 'Отмена', self)
        undoAction.setShortcut('Ctrl+Z')
        undoAction.triggered.connect(self.undo)

        redoAction = QAction(
            QIcon(self.HOME + 'images/redo.png'), 'Восстановить', self)
        redoAction.setShortcut('Ctrl+Shift+Z')
        redoAction.triggered.connect(self.redo)

        zoomInAction = QAction(
            QIcon(self.HOME + 'images/zoomIn.png'), 'Увеличить', self)
        zoomInAction.setShortcut('Ctrl++')
        zoomInAction.triggered.connect(self.zoomIn)

        zoomOutAction = QAction(
            QIcon(self.HOME + 'images/zoomOut.png'), 'Уменьшить', self)
        zoomOutAction.setShortcut('Ctrl+-')
        zoomOutAction.triggered.connect(self.zoomOut)

        settingsAction = QAction(
            QIcon(self.HOME + 'images/settings.png'), 'Настройки', self)
        settingsAction.setShortcut('F9')
        settingsAction.triggered.connect(self.showSettings)

        interpreterAction = QAction(QIcon(
            self.HOME + 'images/interpreter.png'), 'Запустить Python интепритатор', self)
        interpreterAction.setShortcut('F10')
        interpreterAction.triggered.connect(self.interpreter)

        terminalAction = QAction(
            QIcon(self.HOME + 'images/terminal.png'), 'Запустить терминал', self)
        terminalAction.setShortcut('F11')
        terminalAction.triggered.connect(self.terminal)

        runAction = QAction(
            QIcon(self.HOME + 'images/run.png'), 'Запустить', self)
        ...
        self.toolbar.setStyleSheet('''
            QToolButton::hover { background-color: purple;}
        ''')
        ...
        self.searchButton = QPushButton(
            QIcon(self.HOME + 'images/search.png'), 'Поиск', self)
    ...
    def new(self):
        ...
        self.notebook.newTab(editor, index)
    ...
    def open(self):
        ...
        filename = dialog.getOpenFileName(self, "Сохранить")
        ...
        if filename[0]:
            ...
            try:
                ...
                self.notebook.setCurrentIndex(x - 1)
        ...
        if not filename:
            ...
            try:
                with open(filename, 'w') as file:
                    file.write(text)
                    self.statusBar.showMessage(f'{filename} сохранено', 3000)
        ...
        if filename[0]:
            ...
            try:
                with open(fullpath, 'w') as file:
                    file.write(text)
                    self.statusBar.showMessage(fullpath + " сохранено", 3000)
            ...
            self.statusBar.showMessage('Файл не сохранен! !', 3000)
    ...
    def run(self):
        ...
        if not self.textPad.filename:
            self.statusBar.showMessage("Не возможно запустить !", 3000)
            return
    ...
    def onSearch(self):
        ...
        if text == '':
            self.statusBar.showMessage("Введите хотя бы одно слово", 3000)
            return
        x = self.textPad.findFirst(
            text, False, True, False, True, True)  # case sensitive

        if x == False:
            l = len(self.searchEdit.text())
            self.searchEdit.setSelection(0, l)
            self.searchEdit.setFocus()
            self.statusBar.showMessage('<' + text + '> не найден', 3000)
        else:
            self.textPad.setFocus()
    ...
    def centerOnScreen(self):
        ...
        self.move((res.width() - self.frameSize().width()) // 2,
