...
class FileBrowser(QTreeView):
    ...
    def onInfo(self):
        ...
        text += 'Размер:\t' + str(s) + ' bytes' + '\n'
        text += 'Владелец:\t' + owner + '\n'
        text += 'Создан:\t' + str(created.date().day()) + '.' + \
    ...
    def onCopy(self):
        ...
        if fileName == '..':
            ...
            self.mainWindow.statusBar.showMessage(
                "Не удалось скопировать элемент !", 3000)
        ...
        if fileDir:
            ...
            self.mainWindow.statusBar.showMessage('Патч: <' +
                                                  self.copySourceFileName +
                                                  '> отмечен', 3000)
        else:
            self.copySourceFilePath = filePath
            self.copySourceFileName = fileName
            self.mainWindow.statusBar.showMessage('Файл: <' +
                                                  self.copySourceFileName +
                                                  '> отмечен', 3000)
    ...
    def onPaste(self):
        ...
        if not self.copySourceFilePath:
            self.mainWindow.statusBar.showMessage('Ничего не выделено !', 3000)
            return
        ...
        if fileDir and self.isCopyFileFolder:
            ...
            if self.copySourceFileName in fileList:
                q = MessageBox(QMessageBox.Warning, "Ошибка",
                               "Другой путь с данным названием уже существует.",
            ...
            if self.copySourceFilePath in newPath:
                ...
                q = MessageBox(QMessageBox.Critical, 'Ошибка',
                               'Имя пути уже существует в новом пути',
                               QMessageBox.NoButton)
            ...
            try:
                ...
                self.mainWindow.statusBar.showMessage('Выполнено !', 3000)
            ...
            if self.copySourceFileName in fileList:
                result = MessageBox(QMessageBox.Warning, "Внимание",
                                    "Файл уже существует.\n" +
            ...
            if self.copySourceFileName in fileList:
                q = MessageBox(QMessageBox.Information, "Ошибка",
                               "Другой путь с данным названием уже существует.",
                               QMessageBox.NoButton)
                q.exec_()
    ...
    def onRename(self):
        ...
        dialog = EnterDialog(self.mainWindow, fileName,
                             filePath, fileDir, fileInfo, True)
        dialog.exec_()
    ...
    def onDelete(self):
        ...
        if fileDir:
            if fileName == '..':
                self.mainWindow.statusBar.showMessage(
                    'Не удалось удалить элемент !', 3000)
                return
            else:
                result = MessageBox(QMessageBox.Warning, 'Удаление директории', '<b>' + filePath + '</b>' + "  ?",
                ...
                if (result.exec_() == QMessageBox.Yes):
                    try:
                        ...
                        q = MessageBox(QMessageBox.Critical,
                                       "Ошибка", str(e), QMessageBox.NoButton)
                        q.exec_()
            ...
            result = MessageBox(QMessageBox.Warning, 'Удаление файла', path + "<b>" + file + "</b>" + "  ?",
                                QMessageBox.Yes | QMessageBox.No)
